...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    $admin_add = $admin_edit = $admin_delete = 0;
}
?>
...
<body>
    ...
    <h1 class="title">Список администраторов</h1>
    <div class="spacer"></div> <!-- Пустой блок для выравнивания -->
        ...
        <!-- Кнопка "Добавить администратора" -->
            <?php if ($admin_add): ?>
            <a href="add_admin.php" class="btn btn-add">
        ...
        <table>
            ...
            $query = "SELECT * FROM site_admins ORDER BY level DESC, appointed_date ASC"; 
            ...
            <tr>
                ...
                <td>
                            <a href="admin_punish.php?id=<?= $row['id'] ?>" class="btn btn-warning">Посмотреть</a>
                </td>
                <td>
                <?php if ($admin_edit): ?>
                    <a href="edit_admin.php?id=<?= $row['id'] ?>" class="btn btn-edit"><i class="fa fa-pen"></i></a>
                <?php else: ?>
                      <!--p>У вас нет прав на редактирование этого администратора.</p-->
                <?php endif; ?>
    	
                 <?php if ($admin_delete): ?>
                    <a href="delete_admin.php?id=<?= $row['id'] ?>" class="btn btn-delete" onclick="return confirm('Удалить администратора?')"><i class="fa fa-trash"></i></a>
                 <?php else: ?>
