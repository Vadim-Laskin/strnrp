...
<body>
    ...
    <h1 class="title">Список лидеров</h1>
    <div class="spacer"></div> <!-- Пустой блок для выравнивания -->
</div>
        <!--a href="add_leader.php" class="btn btn-add"><i class="fa fa-plus"></i> Добавить</a-->
        <!-- Кнопка "Добавить администратора" -->
            <?php if ($leader_add): ?>
            <a href="add_leader.php" class="btn btn-add">
                <i class="fa fa-plus"></i> Добавить лидера
            </a>
            <?php endif; ?>
        </div>
        <table>
            ...
            <?php
            // Запрос для получения всех лидеров
            $query = "SELECT * FROM site_leader ORDER BY id ASC";
            ...
            ?>
            <tr>
                <td><?= $counter ?></td> <!-- Выводим счетчик -->
                <td><?= $row['name'] ?></td>
                <td><?= $row['organization'] ?></td>
                <td>
                    <?php if ($vk_link): ?>
                        <a href="<?= $vk_link ?>" class="vk-link" target="_blank"><?= $row['vk_name'] ?></a>
                    <?php else: ?>
                        Нет ссылки
                    <?php endif; ?>
                </td>
                ...
                <td>
              	<?php if ($leader_edit): ?>
                    <a href="edit_leader.php?id=<?= $row['id'] ?>" class="btn btn-edit"><i class="fa fa-pen"></i></a>
                  <?php else: ?>
                        <!--p>У вас нет прав на редактирование этого администратора.</p-->
                  <?php endif; ?>
                  <?php if ($leader_delete): ?>
                    <a href="delete_leader.php?id=<?= $row['id'] ?>" class="btn btn-delete" onclick="return confirm('Удалить лидера?')"><i class="fa fa-trash"></i></a>
                  <?php else: ?>
                        <!--p>У вас нет прав на редактирование этого администратора.</p-->
                  <?php endif; ?>
                </td>
